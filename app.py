...
if st.session_state["authentication_status"]:
    ...
    if pagina_atual == "Dashboard":
        ...
        col1, col2, col3 = st.columns(3) # Exemplo com 3 colunas, ajuste conforme necessário

        with col1: # Card de Total de Clientes
            total_clientes = database.get_total_clients_count()
            st.metric(label="👥 Total de Clientes", value=total_clientes)

        # with col2: # Exemplo de outro card que você pode adicionar no futuro
            total_instrutores = database.get_all_instructors() # Você precisaria criar essa função
            st.metric(label="🧑‍🏫 Total de Instrutores", value=total_instrutores)
            total_perinstrutor = database.get_active_client_count_per_instructor()
            st.metric(label="💡 Total de Instrutores per Cliente", value=total_perinstrutor)
